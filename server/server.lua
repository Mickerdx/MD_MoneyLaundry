...
AddEventHandler("laundry:startwasher", function(data)
    ...
    if not washers[data.id].washing then 
        ...
        TriggerClientEvent('QBCore:Notify', src,string.format('info.money_timer'),
        'info', 2500)
        wash(data.id)
    else 
        TriggerClientEvent('QBCore:Notify', src,string.format('error.already_started'),
        'error', 2500)
    end
end)
...
AddEventHandler("laundry:collect", function(data)
    ...
    if washers[data.id].pickup then 
        if washers[data.id].cleaned > 0 then
            player.Functions.AddMoney("cash", washers[data.id].cleaned, "Geld Gewassen")
            washers[data.id].cleaned = 0
            washers[data.id].pickup = false
            washers[data.id].washing = false
        else 
            TriggerClientEvent('QBCore:Notify', src,string.format('error.no_clean_money'),
        'error', 2500)
            washers[data.id].cleaned = 0
            washers[data.id].pickup = false
            washers[data.id].washing = false
        ...
        TriggerClientEvent('QBCore:Notify', src,string.format('error.already_washing'),
        'error', 2500)
    end
end)
